...
import { generateId, isSemanticDuplicate, findNodePath } from '../../utils/mindmapHelpers';
...
export const AddNodeSkill: Skill = {
  ...
  handler: async (args, context) => {
    ...
    const duplicateSibling = siblings.find((s: MindMapNode) => isSemanticDuplicate(s.content, content));
    if (duplicateSibling) {
    ...
    const { root } = context;
    if (root) {
      const path = findNodePath(root, parentId);
      if (path && path.length >= 2) {
        const grandParent = path[path.length - 2];
        const uncles = grandParent.children.filter((c: MindMapNode) => c.id !== parentId);
        const duplicateUncle = uncles.find((u: MindMapNode) => isSemanticDuplicate(u.content, content));
